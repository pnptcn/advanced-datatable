...
export const Table = () => {
    ...
    class DataTable extends HTMLElement {
        ...
        createMiniChart(sanitizedHeader: string) {
            ...
            switch (columnType) {
                case 'number':
                    chartData = this.binNumericData(columnData);
                    break;
                case 'datetime':
                    chartData = this.binDatetimeData(columnData.map(value => new Date(value)));
                    break;
                case 'string':
                    chartData = this.binStringData(columnData);
                    break;
                default:
                    logger.warn(`Unsupported column type: ${columnType} for header "${sanitizedHeader}"`);
                    return;
            }
